...
Describe "Template Integration Tests" {
    ...
    BeforeAll {
        ...
        # Define paths using the initialized environment
        $script:ProjectPath = $script:TestEnvironment.ModuleRoot
        ...
        $script:BackupBaseDir = $script:TestEnvironment.TestBackup
        $script:RestoreBaseDir = $script:TestEnvironment.TestRestore
        $script:TempInstalledAppsDir = Join-Path $script:TestEnvironment.Temp "InstalledAppsSimulation"

        # The initializer already creates the necessary root directories.
        # We only need to create specific sub-directories if the tests require them.
...
"@
    ...
    AfterAll {
        ...
        # The Remove-WmrTestEnvironment function will handle directory cleanup.
        # Unmocking is important.
        [Pester.Mocking.Mock]::UnmockAll()

        # Revert registry changes made during tests
        if (Test-Path "HKCU:\Control Panel\Desktop\Test") {
