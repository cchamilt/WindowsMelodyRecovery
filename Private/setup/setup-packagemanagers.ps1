...
if (!$isAdmin) {
    Write-Host "This script requires administrator privileges to install package managers." -ForegroundColor Red
    exit 1
}
...
if (!(Get-Command scoop -ErrorAction SilentlyContinue)) {
    ...
    try {
        # Scoop needs to be installed as non-admin
        $currentPrincipal = New-Object Security.Principal.WindowsPrincipal([Security.Principal.WindowsIdentity]::GetCurrent())
        $currentUser = $currentPrincipal.Identity.Name
        $userProfile = (Get-CimInstance -ClassName Win32_UserProfile | Where-Object { $_.SID -eq ([System.Security.Principal.WindowsIdentity]::GetCurrent()).User.Value }).LocalPath
...
Write-Host "`nPackage manager setup complete!" -ForegroundColor Blue
